...
with open(r'C:\\Launch','r') as f:
    LAUNCH_PATH = f.read()

if os.path.exists(r"C:\testing"):
    isTesting=True
else:
    isTesting=False

print('Is Testing -> ',isTesting)

EXE_LOCATION  = LAUNCH_PATH + r"\System\\" + r"ImageScripter\EXE"
if EXE_LOCATION not in sys.path:
    sys.path.append(EXE_LOCATION)


#print(Full_Project_Path)
Full_Project_Path = LAUNCH_PATH + r"\Projects\_elan_\elan"
#print(Full_Project_Path)
...
Databases_On_Reports_Location = REPORTS_LOCATION + '\Databases'
...
#Dashboard_Server_IP = '172.16.42.56'
#Dashboard_Server_IP = '172.16.42.30'
Dashboard_Server_IP = '127.0.0.1'
Dashboard_Server_PORT = 65535
...
Projects_Location = PartOne + r"\Projects"
IMAGESCRIPTERPATH = PartOne + r"\System\ImageScripter"
PROJECT = project_path + '\\' + The_NAME


config_missing_var = PROJECT + r"\Data\config_missing_var.dat"
configdate = PROJECT + r"\Data\config.dat"


IntefaceFiles = PROJECT + r"\Data\IntefaceFiles"
...
raw_buildtxt = LAUNCH_PATH + r"\Projects\_elan_\Settings\raw_build.txt"


Converted_Temp_Path = LAUNCH_PATH + r"\System\Data\Converted_Temp"

Converted_HTML_File = LAUNCH_PATH + r"\System\Data\converted.php"

Git_Log_Txt = LAUNCH_PATH + r"\System\Data\gitlog.txt"

Save_System_And_Project = LAUNCH_PATH + r"\System\ImageScripter\EXE\Save_System_And_Project.py"

Git_Bool_Path = LAUNCH_PATH + r"\Projects\_elan_\Settings\Git_Bool.txt"



try:
    with open(Git_Bool_Path, 'r') as f:
        Git_Sync = eval(f.read())
except:
...
Stop_At_End_Of_Loop_Path = LAUNCH_PATH + r"\Projects\_elan_\Settings\Stop_At_End_Of_Loop.txt"

try:
    with open(Stop_At_End_Of_Loop_Path, 'r') as f:
        Stop_At_End_Of_Loop = eval(f.read())
except:
    with open(Stop_At_End_Of_Loop_Path, 'w') as f:
        f.write('False')
    with open(Stop_At_End_Of_Loop_Path, 'r') as f:
        Stop_At_End_Of_Loop = eval(f.read())

with open(Stop_At_End_Of_Loop_Path, 'r') as f:
    Stop_At_End_Of_Loop_Is_On = eval(f.read())
...
Reset_System_And_Project = LAUNCH_PATH + r"\System\ImageScripter\EXE\Reset_System_And_Project.py"
pullpushforloop = LAUNCH_PATH + r"\System\ImageScripter\EXE\pullpushforloop.py"
...
ssh_port = '22'
FAIL_PHP_FILE = LAUNCH_PATH + r"\System\ImageScripter\Lib\site-packages\Database_Manager\Graphs\fail.php"
PASS_PHP_FILE = LAUNCH_PATH + r"\System\ImageScripter\Lib\site-packages\Database_Manager\Graphs\pass.php"
Sandbox_location = LAUNCH_PATH + r"\System\Data\Upload_Sandbox"
Server_Videos_Folder_Location = r"/var/www/html/videos/"
Server_Graph_Folder_Location = r"/var/www/html/graphs/"


Server_Entries_Folder_Location = r"/var/www/html/Entries/"
Server_All_Videos_Folder_Location = r"/var/www/html/All_Videos/"



Fails_ROOT = r"/var/www/html/fails/"
PASSES_ROOT = r"/var/www/html/passes/"
Error_Text_Path = LAUNCH_PATH + r"\System\Data\error_for_graph.txt"
Dictionary_Text_Path = LAUNCH_PATH + r"\System\Data\dictionary.txt"


Email_Alert_Recipients_For_Graph_Failure = ["kennyshay123@gmail.com","elantestertools@gmail.com","ken.shay@nortekcontrol.com"]


FileNameBackuptxt22 = LAUNCH_PATH + r"\Projects\_elan_\Settings\File_Name_Backup.txt"

Database_Backups_Folder = LAUNCH_PATH + r"\Projects\_elan_\Settings\Database_Backups"


lastselectedhubbuildlocationtxt = LAUNCH_PATH + r"\Projects\_elan_\Settings\Last_Selected_Hub_Build_Folder_Location.txt"


Reset_Systempy = LAUNCH_PATH + r'\System\ImageScripter\Lib\site-packages\reset\Reset_System_Files.py'


Reset_Projectpy = LAUNCH_PATH + r'\System\ImageScripter\Lib\site-packages\reset\Reset_Project_Files.py'





Save_Systempy = LAUNCH_PATH + r'\System\ImageScripter\Lib\site-packages\reset\Save_System_Files.py'


Save_Projectpy = LAUNCH_PATH + r'\System\ImageScripter\Lib\site-packages\reset\Save_Project_Files.py'
...
if The_NAME == 'elan':
    try:
        with open(Controller_Type_Location,'r') as f:
            Controller_Type = f.read()
    except:
        Controller_Type = 'SC10'
...
Clear_File_Script_Writer = IMAGESCRIPTERPATH + r"\Lib\site-packages\imagescripter\core\clear.txt"
...
Temp_Data_location = PartOne + "\System\Data\Temp\\"



Dashboard_Client_Widget_Ui = IMAGESCRIPTERPATH + r"\Lib\site-packages\Dashboard_2\client_widget.ui"
Dashboard_Ui = IMAGESCRIPTERPATH + r"\Lib\site-packages\Dashboard_2\dashboard.ui"




last_selected_app_for_get_image_in_writer_textfile = PartOne + r"\System\Data\last_selected_app_for_get_image_in_writer.txt"
...
try:
    with open(Active_Script_Ouput,'r') as f:
        f.close()
except:
...
Hub_Icon_For_Tools = IMAGESCRIPTERPATH + r"\Lib\site-packages\Hub\cog.icns"
...
if isTesting == False:
    ...
    Database_Folder = Desktop_location + "\Databases"
if isTesting == False:
    Dictionary_Database_Folder = REPORTS_LOCATION + "\Dictionary_Databases"
else:
    Dictionary_Database_Folder = Desktop_location + "\Dictionary_Databases"

MAX_CUTOFF = -20


try:
    Controler_Build_Location_txt = PartOne + "\System\Data\\" + "Controler_Build_Location.txt"#
    with open(Controler_Build_Location_txt,'r') as f:
        Controler_Build_Location = f.read()
except:
    with open(Controler_Build_Location_txt, 'w') as f:
        f.write(REPORTS_LOCATION + "\Elan_Builds")
    with open(Controler_Build_Location_txt,'r') as f:
        Controler_Build_Location = f.read()
...
if isTesting == False:
        try:
            ...
            message = "Eluminate can't get to build location... \n\n{}\n\n Please enable this location in Windows and restart Eluminate. Eluminate will be using build 8_4_96_0 for now. You may encounter unpredictable issues until you fix this. Or you can change the build location".format(Controler_Build_Location)
...
Graph_Folder_Location = LAUNCH_PATH + r'\System\Data\graphs'
if not os.path.isdir(Graph_Folder_Location):
    os.mkdir(Graph_Folder_Location)

Database_Location = Database_Folder + '\\' + Station_Name + '.db'

Time_Script_Log_ = r"\\172.16.43.21\nas\Reports\Log\Time_Script_Log.txt"


Ignore_Graph_Script_List = ['XXfunc','XXkeeponinterface','func','keeponinterface']





exclude_directories_from_missing_scripts = ['Frank','TimeCheck-Two-SC100',
                                                'TimeCheck-All_Controllers',
                                                'TimeCheck',
                                                'Paul_Scripts',
                                                'Android_For_Frank',
                                                'Add_All',
                                                'Long_Tests',
                                                'Jeffs on Hold DoorStation',
                                                'Jeffs_On_Hold',
                                                'Ip_Amp_Dante',
                                                'Door_Station_Repeat_Discovery',
                                                'Ip_Amp_System',
                                                'Ip_Amp_Dante_System',
                                                'Test',
                                                'Test_Station',
                                                'Tonys_Scripts',
                                                'Victors_Scrips',
                                                'Video_Intercom_Loop'] # directory (only names) want to exclude



Dictionary_Database_Location = Dictionary_Database_Folder + '\\' + Station_Name + "__" + Active_Build_without_EXE + '.json'
...
if isTesting == False:
    ...
    Videos_For_Database_Location = Desktop_location + r"\videos"



Images_For_Database_Location = REPORTS_LOCATION + r"\Images"
...
All_Elan_New_Builds_Folder = r'\\172.16.43.10\builds\ELAN'


lang_default_for_scriptxml = LAUNCH_PATH + r"\Projects\_elan_\elan\lang_default_for_script.xml"
...
Default_Radio_Button_State_File_Location = Settings_Location + r"ScriptRunner_RadioButtons\default.txt"


on_fail_go_to_next_folder_Radio_Button_State_File_Location = Settings_Location + r"ScriptRunner_RadioButtons\on_fail_go_to_next_folder.txt"

run_on_start_radio_button_file_state_location = Settings_Location + r"ScriptRunner_RadioButtons\Run_on_Start.txt"



Stop_on_Fail_radio_button_file_state_location = Settings_Location + r"ScriptRunner_RadioButtons\Stop_on_Fail.txt"





Script_Runner_Controllers_Folder = Settings_Location + r"ScriptRunner_Controllers"#
if not os.path.exists(Script_Runner_Controllers_Folder):
    os.makedirs(Script_Runner_Controllers_Folder)



Script_Runner_Folders_Folder = Settings_Location + r"ScriptRunner_Folders"
if not os.path.exists(Script_Runner_Folders_Folder):
    os.makedirs(Script_Runner_Folders_Folder)




Script_Runner_RadioButtons_Folder = Settings_Location + r"ScriptRunner_RadioButtons"
if not os.path.exists(Script_Runner_RadioButtons_Folder):
    os.makedirs(Script_Runner_RadioButtons_Folder)





Script_Runner_Scripts_Folder = Settings_Location + r"ScriptRunner_Scripts"
if not os.path.exists(Script_Runner_Scripts_Folder):
    os.makedirs(Script_Runner_Scripts_Folder)

clear_text_editors = Settings_Location + r"clear_texteditor.txt"
try:
    with open(clear_text_editors,'r') as f:
        a = f.read()
except:
    with open(clear_text_editors,'w') as f:
        f.write('a')

old_lang_default_for_scripttxt = LAUNCH_PATH + r"\Projects\_elan_\elan\old_lang_default_for_script.txt"

bluescreenpng = LAUNCH_PATH + r"\Projects\_elan_\elan\Data\bluescreen.png"

Hub_Ui = LAUNCH_PATH + r"\System\ImageScripter\Lib\site-packages\Hub\hub.ui"
Apps_Ui = LAUNCH_PATH + r"\System\ImageScripter\Lib\site-packages\Hub\apps.ui"
Add_New_Controller_Ui = LAUNCH_PATH + r"\System\ImageScripter\Lib\site-packages\Script_Runner\Add_New_Controller.ui"
Script_Runner_Gui_Location = LAUNCH_PATH + r"\System\ImageScripter\Lib\site-packages\Script_Runner\script_runner_gui.py"
...
Clone_Ui = LAUNCH_PATH + r"\System\ImageScripter\Lib\site-packages\Hub\clone.ui"
Wizard_Ui = LAUNCH_PATH + r"\System\ImageScripter\Lib\site-packages\Wizard\wizard.ui"
Name_Clone_Ui = LAUNCH_PATH + r"\System\ImageScripter\Lib\site-packages\Hub\name_clone.ui"
...
Python_Exe_Location_Without_Window = ImageScripter_Location + 'pythonw.exe'#
cygdrivetwo = PROJECT_Reverse + "/"#####Reverse###
COutputoutputcsv = PROJECT + r"\Data\output.csv"

Status_For_Uploading = PROJECT + r"\Data\status_for_uploading.txt"
...
Active_Script_For_Script_Runner = ImageScripter_Location + "Lib\site-packages\Script_Runner\Data\Active_Script.txt"
Data_Dir = ImageScripter_Location + "Lib\site-packages\Script_Runner\Data"
...
diplay_image_for_capturepy_Any = Python_Exe_Location + ' ' + ImageScripter_Location + r"Lib\site-packages\Script_Writer\diplay_image_for_capture.py"
...
diplay_image_for_capturepyAndroid = Python_Exe_Location + ' ' + ImageScripter_Location + r"Lib\site-packages\Script_Writer\diplay_image_for_capture.py Android"
...
try:
    with open(Templet_Image_File_With_Red_Box,'r') as f:
        f.close()
except:
    with open(Templet_Image_File_With_Red_Box,'w') as f:
        f.write('default file')
...
try:
    with open(screenshot_file_itp8,'r') as f:
        f.close()
except:
    with open(screenshot_file_itp8,'w') as f:
        f.write('default file')
...
Show_Image_Ui_Script_Runnner_Location = ImageScripter_Location + r"Lib\site-packages\imagescripter\core\show_image.ui"#



Default_Loop = ImageScripter_Location + r"Lib\site-packages\Script_Runner\Loops\Default_Loop.py"
Run_Next_Folder_On_Fail_Loop = ImageScripter_Location + r"Lib\site-packages\Script_Runner\Loops\Run_Next_Folder_On_Fail_Loop.py"

ImShow_ScriptRunner = ImageScripter_Location + r"Lib\site-packages\Script_Runner\ImShow_ScriptRunner.ui"
