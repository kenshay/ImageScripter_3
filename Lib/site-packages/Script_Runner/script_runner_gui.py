...
class Update_Controller_Info_Thread(QThread):
    def __init__(self,parent):
        ...
        while True:
            ...
            try:
                ...
                with open(Total_Runs_Script_Runnertxt, 'r') as f:
                    ...
                    self.parent_ui.total_runs.setText('Total Cycles = ' + scritp_runner_cycles)
            except Exception as e:
                print(e)
            sleep(10)
...
class Script_Runner_Gui_Class(QWidget):
    def __init__(self,Run_On_Start = False):
            ...
            #Update_Controller_Info_And_Cycle_Thread = Update_Controller_Info_Thread(self.ui)
    ...
    def load_image_picture_changed(self,Image_Path):
        try:
            ...
            self.ui.label_4.setMaximumSize((Width/10)*7,16777215)
            self.ui.label_4.setPixmap(Pixmap)
            self.ui.label_4.resize(Pixmap.width(), Pixmap.height())
            Pixmap2 = QtGui.QPixmap(Target_Path)
            Pixmap2 = Pixmap2.scaled(200, 100, QtCore.Qt.KeepAspectRatio)
            self.ui.label_6.setMaximumSize((Width/10)*7,16777215)
            self.ui.label_6.setPixmap(Pixmap2)
            self.ui.label_6.resize(Pixmap2.width(), Pixmap2.height() - Pixmap.height())
            try:
                with open(Current_Running_Script,'r') as f:
                    txt = f.read()
                self.ui.label_7.setText(txt)
            except:
                self.ui.label_7.setText('')
            try:
                with open(Current_Action,'r') as f:
                    txt = f.read()
                self.ui.label_8.setText(txt + '... ')
            except:
            ...
            try:
                ...
                txt = PercentLong[:2]
                if txt != '':
                    self.ui.label_9.setText(txt + '% MATCH')
                else:
