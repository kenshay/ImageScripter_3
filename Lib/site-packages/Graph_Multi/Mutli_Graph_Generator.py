...
from Database_Manager.Dictionary_Database_Manager import Dictionary_Database_Manager_Class,is_there_any_Scripts_entries_for_build_and_station
...
def Make_LOG_For_Station(Dictionary_Database_Manager,BRANCH,BUILD,Station):
    ...
    with tag('html', lang=" en"):
        ...
        with tag('body'):
            with tag('table'):
                with tag('tr'):
                    for title in ['Index','Date','Start','End','Station','Build','Pass','Memory_Start','Memory_End','Script']:
                        with tag('td'):
                            text(title)
...
def Create_Index_Page():
    ...
    with tag('html', lang=" en"):
        with tag('head'):
            doc.asis('<!-- basic.html -->')
            doc.asis('<meta http-equiv="refresh" content="300">')
            with tag('title'):
                text('basic.html')
            doc.asis('<meta charset = "UTF-8" />')
            doc.asis('<link rel="stylesheet" href="../indexstyle.css">')
            with tag('style'):
                doc.asis('div {'
            ...
            #doc.asis('<link rel="stylesheet" href="style.css">')
        with tag('body'):
            link = "https://github.com/kenshay/Eluminate_Installer/archive/master.zip"

            doc.asis('<a href="{}">{}</a>'.format(link,"Install Eluminate"))
            for i in range(3):
                with tag('p'):
                    text('')
            ...
            for BRANCH in reversed(Branches):
                ...
                for station_name in sorted(Stations_List):
                    if is_there_any_Scripts_entries_for_build_and_station(BRANCH, station_name) == True:
                        with tag('table'):
                            with tag('tr'):
                                with tag('h2'):
                                    text(station_name + ' (' + BRANCH + ')')
                            for build in Last_Five_Build:
                                if is_there_any_Scripts_entries_for_build_and_station(build,station_name) == True:
                                    with tag('td'):
                        ...
                        with tag('script'):
                            ...
                            var rawFile = new XMLHttpRequest();
                                rawFile.open("GET", "status/@name@", false);
                                rawFile.onreadystatechange = function ()
                                {
                                    if(rawFile.readyState === 4)
                                    {
                                        if(rawFile.status === 200 || rawFile.status == 0)
                                        {
                                            ...
                                            document.getElementById("@theid@").innerHTML = allText;
                                        }
                                    }
                                }
                                rawFile.send(null);
                            """
                            TheTEXT = TheTEXT.replace("@name@",station_name)
                            TheTEXT = TheTEXT.replace("@theid@",station_name)
                            doc.asis(TheTEXT)
                        ###################################################
    ...
    for BRANCH in Branches:
        ...
        for Station_Name in sorted(Stations_List):
            for BUILD in Last_Five_Builds:
                if is_there_any_Scripts_entries_for_build_and_station(BUILD,Station_Name):
                    Underscore_Build = BUILD.replace('.', '_')
