...
class SSH_Manager_Class():
    ...
    def Rename_Duplicate_A_File(self):
    ...
    def get_percent_of_harddrive_In_Use(self):
        #print('Getting Memory In Use')
        for i in range(1):
            try:
                #print('get_percent_of_memory_In_Use')
    ...
    def get_amount_of_ssh_connections_in_use(self):
        if Controller_Type == 'SC100':
            #print('Getting Memory In Use')
            for i in range(1):
                try:
    ...
    def get_Build(self):
        try:
            ...
            scp.get('/ELAN/CONFIG/SYSTEM.DAT', Build_Raw_File)
            ssh.close()
            scp.close()
            junk_list = []
            with io.open(Build_Raw_File, 'r', encoding='cp1252', errors='ignore') as f:
                for line in f:
    ...
    def get_Controller_Name(self):
        try:
            ...
            ssh = createSSHClient()
            scp = SCPClient(ssh.get_transport())
            scp.get('/ELAN/CONFIG/SECURE.DAT', Name_Raw_File)
            ssh.close()
            scp.close()
            junk_list = []
            with io.open(Name_Raw_File, 'r', encoding='cp1252', errors='ignore') as f:
                for line in f:
                    junk_list.append(line)
            whole_string = junk_list[0]
...
while True:
    try:
        space = SSH_Manager.get_percent_of_harddrive_In_Use()
        print(space)
    except:
        pass
#    print('\n')
