...
class Database_Manager_Class():
    ...
    def enter_results_into_database(self,Index_Time, Script_Start_Time, Steps, Date, Script_End_Time, Script_Name, Pass, Fail, Skip,Controller_Name,Station_Name, Controller_Build, Error, target_img, templet_img, Error_ID,Fail_Video,Pass_Video,Client):
        print('Entering Results Into Database -> ',self.database_location_file)
        conn = sqlite3.connect(self.database_location_file)
        c = conn.cursor()
        c.execute('''INSERT INTO Reports(Index_Time,Script_Start_Time,Steps, Date,Script_End_Time, Script_Name, Pass, Fail, Skip, Controller_Name, Station_Name, Controller_Build,Error,target_img,templet_img,Error_ID,Fail_Video,Pass_Video,Client)
                                  VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
                  (Index_Time, Script_Start_Time, Steps, Date, Script_End_Time, Script_Name, Pass, Fail, Skip,Controller_Name,Station_Name, Controller_Build, Error, target_img, templet_img, Error_ID, Fail_Video, Pass_Video, Client))
        conn.commit()
        conn.close()
    ...
    def enter_test_results_into_database(self):
        ...
        conn = sqlite3.connect(self.database_location_file)
        c = conn.cursor()
        c.execute('''INSERT INTO Reports(Index_Time,Script_Start_Time,Steps, Date,Script_End_Time, Script_Name, Pass, Fail, Skip, Controller_Name, Station_Name, Controller_Build,Error,target_img,templet_img,Error_ID)
                                  VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
                  (Index_Time, Script_Start_Time, Steps, Date, Script_End_Time, Script_Name, Pass, Fail, Skip,Controller_Name,Station_Name, Controller_Build, Error, target_img, templet_img, Error_ID))
        conn.commit()
        conn.close()
