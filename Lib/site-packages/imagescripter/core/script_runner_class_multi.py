    ...
    if "ALERT" in scriptname:
        for i in range(100):
            try:
                ...
                msg['From'] = "elantestertools@gmail.com"
                msg['To'] = ", ".join(Email_Recipients)
                msg['X-Priority'] = '2'
                s = smtplib.SMTP("smtp.gmail.com", 587)
                s.ehlo()
                s.starttls()
                s.login("elantestertools@gmail.com", "corebrands123")
                s.send_message(msg)
                s.close()
                break
...
def Create_Controller_List_File_If_Not_Exists():
    File_Location = r"C:\Settings\Run_Controller_List_Script_Runner.txt"
    try:
...
def Change_Controller_Information(Name,Password,IP):
    print('Changing Controller Information')
    Variable_File_Getter.setFileFromVariable('Controller_Name',Name)
    Variable_File_Getter.setFileFromVariable('Controller_IP',IP)
    Variable_File_Getter.setFileFromVariable('Controller_Password',Password)
...
class Script_Runner_Class():
    ...
    def init_run_scripts_while_loop(self):
            Controller_List_Information = Variable_File_Getter.getVariableFromFile('Run_Controller_List_Script_Runner')
            if Controller_List_Information != '':
                print('###########################')
                print(Controller_List_Information)
                print('###########################')
                Controller_List_Information = eval(Controller_List_Information)
            else:
                Controller_List_Information = []
            List_of_opened_apps = getListOfAllApplications()
            List_of_script_objects = self.get_list_of_scripts()
            while True:
                    for specific_controller in Controller_List_Information:
                            Controller_Name = specific_controller[0]
                            Controller_Password = specific_controller[1]
                            Controller_IP = specific_controller[2]
                            Change_Controller_Information(Name = Controller_Name,Password=Controller_Password,IP = Controller_IP)
