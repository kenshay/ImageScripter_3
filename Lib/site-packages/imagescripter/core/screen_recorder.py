...
Videos_For_Database_Location_Folder = Videos_For_Database_Location
...
class Screen_Recorder_Class():
    ...
    recording_directory = Recordings_Location
    def getTime(self):
        now = datetime.now()
    ...
    def stitch_videos_together_as_one(self,left_video,right_video,output_location):
        ...
        self.video_proc = subprocess.Popen(['ffmpeg','-i',left_video,'-i',right_video,'-filter_complex','hstack',output_location],startupinfo=startupinfo,
    ...
    def record(self,script_name = 'default'):
        ...
        #self.video_proc = subprocess.Popen(['ffmpeg','-loglevel','panic','-f','gdigrab', '-framerate','5', '-i','desktop', self.File_name_with_avi,'-b', '1'])



        #self.video_proc = subprocess.Popen(['ffmpeg','-loglevel','quiet','-f','gdigrab', '-framerate','5', '-i','desktop', self.File_name_with_avi,'-b', '1'])
        self.video_proc = subprocess.Popen(['ffmpeg','-loglevel','quiet','-f','gdigrab', '-framerate','5', '-i','desktop', self.File_name_with_avi,'-b', '1'],startupinfo=startupinfo,
                    stderr=subprocess.PIPE, stdout=subprocess.PIPE)
        os.chdir(curdir)
    ...
    def convert_video(self,original_video_path,output_video_path):
        ...
        curdir = os.getcwd()
        os.chdir(Imageexelocation)
        self.video_proc = subprocess.Popen(['ffmpeg','-i',original_video_path,output_video_path,'-hide_banner'],startupinfo=startupinfo,
                    stderr=subprocess.PIPE, stdout=subprocess.PIPE)
        os.chdir(curdir)
        self.video_proc.wait()
    ...
    def encode(self,astring):
        try:
            astring = str(astring)
            print("###########################")
            print('Encoding String -> ',astring)
            print('###########################')
            encoded_string = re.sub("[^0-9]", "", astring)
            print('Encoded String is -> ',encoded_string)
            encoded_string += 'x'
            return encoded_string
        except Exception as e:
            print(e)
            return str('Something_went_wrong')

    def get_video_thumbnail(self):
        import subprocess
        curdir = os.getcwd()

        ##Imageexelocation = r"C:\ImageScripter_2\Lib\site-packages\imagescripter\exe"
            ...
            for i in range(10):
                try:
                    ...
                    if not os.path.exists(new_file_name):
                        raise ValueError("Video Doesnt Exists",new_file_name)
                    break
                except PermissionError as e:
                    print("PermisionError!")
                    print(e)
                    time.sleep(1)
                    if i >= 9:
                        raise
    ...
    def stop(self,Station_Name,Script_Name,Build,PassOrFail,Error):
        try:
            ...
            try:
                ...
                print(e)
                new_file_name_without_avi = Videos_For_Database_Location_Folder + '\\' + Station_Name + '__' + Script_Name + '__' + Build + '__' + PassOrFail + '__' + str(Error)
...
if __name__ == "__main__":
    String = """Traceback (most recent call last):
      ...
      File "C:\ImageScripter_2\lib\shutil.py", line 1156, in copyfile
    ...
    hd_camera.record()
    time.sleep(5)
    video_path  = hd_camera.stop(Station_Name = 'KensDesk',Script_Name = 'test_script',Build = '8.3.6.2',Error = Encoded_Traceback,PassOrFail = 'Pass')
    #print('Video Path ####')
    print(video_path)
