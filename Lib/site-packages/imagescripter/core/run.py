from elan import *
from elan import *
GetBackTOHomeScreenITP8()
tries = 0
for i in range(100):
    Itp8.security10items.Click()
    sleep(5)
    if Itp8.securityblank.Exists() == True:
        Itp8.securityhighlighted.Click()
        Sleep(1)
        if tries >= 50:
            raise ValueError('Blank Security Page')
        else:
            tries += 1
            continue
    else:
        break
if Itp8.doorlockwarning.Exists() == True:
    raise ValueError('Somthing is wrong with the door lock')
	
#if Itp8.doorlockyelling.Exists() == True:
#    raise ValueError('Somthing is wrong with the door lock')	
##################################################Rightdoorlock
Itp8.rightdoorlock.Click()
Sleep(1)
Itp8.eventlog.Wait()
Sleep(1)
Itp8.anysecured.Wait(seconds = 500) #need to wait for log to load.
Sleep(2)
##########################Right Door Lock
try:
    Itp8.battery.Click(xoffset = 950,yoffset = 20)
except:
    Sleep(10)
    Itp8.battery.Click(xoffset=950, yoffset=20)
Sleep(1)
if Itp8.notrespodingdoorlock.Exists() == True:
    raise ValueError("Door lock is not respoding")
Itp8.eventlogunsecured.Wait(threshold = .94)
Sleep(1)
###########################################################################HERE
Itp8.battery.Click(xoffset = 850,yoffset = 20)
Sleep(1)
Itp8.eventlogsecured.Wait(threshold = .93)
Sleep(1)
if Itp8.notrespodingdoorlock.Exists() == True:
    raise ValueError("Door lock is not respoding")
Itp8.shudder.Click()
Sleep(1)
################################################leftdoorlock
Itp8.leftdoorlock.Click()
Sleep(1)
Itp8.eventlog.Wait()
Sleep(1)
Itp8.anysecured.Wait(seconds = 500)#need to wait for log to load.
Sleep(2)
try:
    Itp8.battery.Click(xoffset = 950,yoffset = 20)
except:
    Sleep(10)
    Itp8.battery.Click(xoffset=950, yoffset=20)
Sleep(1)
Itp8.securitypinone.Click()
Sleep(1)
Itp8.securitypinone.Click()
Sleep(1)
Itp8.securitypinone.Click()
Sleep(1)
Itp8.securitypinone.Click()
Sleep(1)
if Itp8.notrespodingdoorlock.Exists() == True:
    raise ValueError("Door lock is not respoding")
Itp8.eventlogunsecuredbyadmin.Wait(threshold = .95)
Sleep(1)
Itp8.battery.Click(xoffset = 850,yoffset = 20)
Sleep(1)
Itp8.eventlogsecured.Wait(threshold = .93)
Sleep(1)
Itp8.shudder.Click()
Sleep(1)
Itp8.security10itemshighlighted.Click()
Sleep(1)
Close_Image_Show()