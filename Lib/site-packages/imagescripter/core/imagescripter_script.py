...
from Graph_Multi.Mutli_Graph_Generator import Upload_To_Server
...
class ImageScripter_Script_Class():
    ...
    def Send_Station_Status_To_Server(self,StationName,list_of_info):
        try:
            with open(Status_For_Uploading,'w') as f:
                for line in list_of_info:
                    f.write(line + '\n')
            dest = r"/var/www/html/status/{}".format(StationName)
            Upload_To_Server(Status_For_Uploading,dest)
        except Exception as e:
            traceback.print_exc()
    ...
    def get_and_enter_database_information_about_script(self):
        ...
        FMT = '%H:%M:%S'
        Duration = datetime2.strptime(Script_End_Time, FMT) - datetime2.strptime(Script_Start_Time, FMT)
        ...
        if Fail == 'True':
            ...
            if Pass_Video == None:
                Pass_Video = 'Too Short For Video'
            target_img = 'None'
            templet_img = 'None'
        ...
        try:
            aList = [('index_time',Index_Time),
                    ('script_start_time','script_start_time'+':'+Script_Start_Time),
                    ('date','date'+':'+Date),
                    ('script_end_time','script_end_time'+':'+Script_End_Time),
                    ('script_name','script_name'+':'+Script_Name),
                    ('pass','pass'+':'+Pass),
                    ('fail','fail'+':'+Fail),
                    ('skip','skip'+':'+Skip),
                    ('script_duration','script_duration'+':'+str(Duration_in_seconds)),
                    ('controller_name','controller_name'+':'+self.Controller_Name),
                    ('station_name','station_name'+':'+self.Station_Name),
                    ('controller_build','controller_build'+':'+Controller_Build),
                    ('target_img','target_img'+':'+New_Target_Path),
                    ('templet_img','templet_img'+':'+New_Template_Path),
                    ('error_id','error_id'+':'+Error_ID),
                    ('fail_video','fail_video'+':'+Fail_Video),
                    ('pass_video','pass_video'+':'+Pass_Video),
                    ('client','client'+':'+Client),
                    ('index_time_string','index_time_string'+':'+Index_Time_String),
                    ('memory_used_start','memory_used_start'+':'+str(Memory_Used_Start)),
                    ('memory_used_end','memory_used_end'+':'+str(Memory_Used_End)),
                    ('data','data'+':'+Data),
                    ('error','error'+':'+Error),
                    ('steps','steps'+':'+Steps)]
            Dictionary_Entry = dict(aList)
            self.Dictionary_Database_Manager.enter_results_into_database(Dictionary_Entry)
            #self.Send_Station_Status_To_Server(self.Station_Name,[Script_End_Time,Date])
    ...
    def run(self):
        ...
        #Script_Name = self.Script_Name.split(',')[-2]
        self.Send_Station_Status_To_Server(self.Station_Name, [self.Script_Name,self.Script_Start_time, Date])
        ######################################################################
        Encoded_Traceback = ' '
...
if __name__ == "__main__":
    ...
    #script = ImageScripter_Script_Class(r"C:\Users\ken.shay\Desktop\00400,PC,Ping,Time Sever,N-A,N-A,Verifies that Apple4444 time server is up,0.py")
    script.run()
    ####################################################################################################################
