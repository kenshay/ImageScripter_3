...
class Editor_Class(QsciScintilla):
    ...
    def contextMenuEvent(self, event):
        try:
            ...
            set_offset_Action = menu.addAction("Get Offset")
    ...
    def Take_New_Image(self,wait = False):
        try:
            #self.mother_widget.ui.pushButton_4.setEnabled(False)
            ...
            if Pass_or_Fail == True:
                ...
                if wait == False:
                    ...
                    os.system(Script_WriterImage_Converterpy + App + ' ' + path)
                    #import subprocess
                    #subprocess.Popen(
                    ...
                    os.system(Script_WriterImage_Converterpy + App + ' ' + path)
                os.chdir(curDur)
            #self.mother_widget.ui.pushButton_4.setEnabled(True)
    ...
    def set_offset(self):
        try:
            ...
            print(run_txt)
            try:
                AppAndImageObject = eval(App + '.' + Image)
                #######################
                AppAndImageObject.Wait(seconds=2)
                import subprocess
                ...
                #Configurator.ghdjfjasdjasdf = Image_Class('C:\ImageScripter_2\Lib\site-packages\elan\Applications_And_Devices\Configurator\ghdjfjasdjasdf.png',Configurator, .96)

                command = App + '.' + Image + ' = ' + 'Image_Class("#path_of_image#", #Interface#, .96)'
                command = command.replace('#path_of_image#',path)
                command = command.replace('#Interface#',App)
                print(command)
                exec(command)
                command = App + '.' + Image + '.Wait(seconds = 2)'
                print(command)
                exec(command)
                #AppAndImageObject.Wait(seconds = 2)
                import subprocess
                ...
                subprocess.Popen(
                    sitepackagesScript_Writeroffset_toolpy,
                    shell=True)
            ...
            #ctypes.windll.user32.MessageBoxW(0, "Please restart Image Writer to use offset on this image. This is a complicated bug that needs to be fixed involving new images\n\n" + str(e), "Error", 1)
