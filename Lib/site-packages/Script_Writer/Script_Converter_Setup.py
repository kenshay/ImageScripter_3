...
class Script_Converter_Setup_Class(QWidget):
    ...
    def old_script_button_clicked(self):
            if 'Windows-7' not in self.get_windows_version():
                ...
                name = QFileDialog.getOpenFileName(parent=self,directory = self.All_Script_Folder,caption=self.tr("Select a file"),filter=self.tr('Image files (*.py)'),)
            else:
                name = QFileDialog.getOpenFileName(None, 'Open File', self.All_Script_Folder, '*', '*', QFileDialog.DontUseNativeDialog)
            if name != '':
                file_name = name[0]
    ...
    def new_script_button_clicked(self):
        if 'Windows-7' not in self.get_windows_version():
            ...
            name = QFileDialog.getSaveFileName(parent=self, directory=self.All_Script_Folder,
                                               caption=self.tr("Select a file"), filter=self.tr('Image files (*.py)'), )
        else:
            name = QFileDialog.getSaveFileName(None, 'Open File', self.All_Script_Folder, '*', '*',
