from paths import *
import sys
from PyQt5.QtWidgets import QApplication, QWidget
from win32api import GetSystemMetrics
from PyQt5.QtGui import QIcon
from PyQt5.QtWidgets import QApplication, QWidget, QInputDialog, QLineEdit, QFileDialog
from PyQt5.QtWidgets import QApplication, QWidget, QPushButton, QMessageBox,QLabel,QTextEdit
from PyQt5 import QtGui, uic, Qt
from PyQt5.QtGui import QStandardItemModel,QStandardItem
import traceback

from variable_file_getter import Variable_File_Getter
import os
from PyQt5.QtGui import QIcon, QPixmap
import base64
import sys
from PyQt5.QtGui import *
from PyQt5.QtWidgets import *
from PyQt5.QtCore import *
from Script_Writer.editor import Editor_Class
from Script_Writer.snipping_tool import Snipping_Tool_Class
from win32api import GetSystemMetrics
from tkinter import filedialog
from PyQt5 import QtCore
from tkinter import *
import shutil
import PyQt5
from win32api import GetSystemMetrics
from PyQt5 import QtCore
import shutil
from Script_Writer.information_getter import Information_Getter
from Script_Writer.variable_file_getter import Variable_File_Getter
import os

##Convert_Script_Path = r"C:\Settings\Converted_Script_Path.txt"
##Convert_Script_Path = r"C:\Settings\Converted_Script_Path.txt"


class Script_Converter_Setup_Class(QWidget):

    def __init__(self,app):
        super().__init__()
        #self.ui = uic.loadUi(r"database_viewer.ui")
        self.app = app
        with open(Convert_Script_Path,'w') as f:
            f.write('None')

        ##Script_WriterScript_Converter_Setupui = r"C:\ImageScripter_2\Lib\site-packages\Script_Writer\Script_Converter_Setup.ui"

        self.ui = uic.loadUi(Script_WriterScript_Converter_Setupui)
        self.ui.setWindowTitle("Script_Converter_Setup")
        ##self.All_Script_Folder = Variable_File_Getter.getVariableFromFile('All_Scripts_Folder_Location')
        self.All_Script_Folder = All_Scripts_Folder_Location
        self.ui.pushButton_6.clicked.connect(self.old_script_button_clicked)
        self.ui.pushButton_3.clicked.connect(self.new_script_button_clicked)
        self.ui.pushButton.clicked.connect(self.save_button_clicked)
        self.old_script = None
        self.new_script = None
        self.ui.pushButton_7.clicked.connect(self.open_old_script)
        self.ui.pushButton_8.clicked.connect(self.open_new_script)

        #self.ui.pushButton_8.clicked.connect(self.old_interface_clicked)
        #self.ui.pushButton_4.clicked.connect(self.new_interface_clicked)
        self.ui.pushButton.clicked.connect(self.save_clicked)
        self.ui.pushButton_2.clicked.connect(self.cancel_clicked)
        self.fill_combo_boxes()
        self.active_file_name = None
        try:
            name = sys.argv[1]
            if name != '':
                self.active_file_name = name
                self.ui.lineEdit_6.setText(self.active_file_name)
            else:
                self.active_file_name = None
        except Exception as e:
            self.active_file_name = None
        self.ui.show()


    def save_button_clicked(self):
        import os
        #self.text_widget = QTextEdit()
        #self.text_widget.showMaximized()
        curdir = os.getcwd()
        old_script = self.ui.lineEdit_6.text()
        new_script = self.ui.lineEdit_3.text()
        if old_script != '' and new_script != '':

            print('Run')

            old_interface = str(self.ui.comboBox.currentText())
            new_interface = str(self.ui.comboBox_2.currentText())
            print(old_script,new_script,old_interface,new_interface)
            os.chdir(self.Apps_Folder)
            import glob
            import os
            import shutil
            dira = old_interface
            dirb = new_interface
            for filename in glob.glob(os.path.join(dira, '*.png')):
                #print(os.path.join(dirb, os.path.basename(filename)))
                if not os.path.isfile(os.path.join(dirb, os.path.basename(filename))):
                    print('Moving -> ',filename,' ',dirb)
                    shutil.copy(filename, dirb)




            #old_interface_list = os.listdir(old_interface)
            #old_interface_list = [x for x in old_interface_list if x.endswith('.png')]
            #new_interface_list = os.listdir(new_interface)
            #new_interface_list = [x for x in new_interface_list if x.endswith('.png')]
            #for pic in old_interface_list:
            #    if pic not in new_interface:
            #        old_pic_path = old_interface + '\\' + pic
            #        new_pic_path = new_interface + '\\' + pic
            #        print(old_pic_path,'-->',new_pic_path)
            #        String = old_pic_path + '-->' + new_pic_path
                    #self.text_widget.append(String)
            #        shutil.copy2(old_pic_path,new_pic_path)
            with open(old_script,'r') as f:
                old_script_text = f.read()
            new_script_text = old_script_text.replace(old_interface,new_interface)
            with open(new_script,'w') as f:
                f.write(new_script_text)
            with open(Convert_Script_Path,'w') as f:
                f.write(new_script)
            self.app.quit()

        else:
            import ctypes
            ctypes.windll.user32.MessageBoxW(0, 'Script paths Cannot be empty', 'Error', 0)










    def fill_combo_boxes(self):
         ##self.Apps_Folder = Variable_File_Getter.getVariableFromFile('Applications_Folder_Location')
         self.Apps_Folder = Applications_Folder_Location
         Lista = os.listdir(self.Apps_Folder)
         self.ui.comboBox.clear()
         self.ui.comboBox_2.clear()
         for i in Lista:
             self.ui.comboBox.addItem(i)
             self.ui.comboBox_2.addItem(i)


    def open_old_script(self):
        try:
            os.system('notepad.exe ' + self.old_script)
        except Exception as e:
            print(e)

    def open_new_script(self):
        try:
            os.system('notepad.exe ' + self.new_script)
        except Exception as e:
            print(e)


    def old_script_button_clicked(self):
            if self.active_file_name != None:


                Directory = os.path.basename(self.active_file_name)
            else:
                Directory = self.All_Script_Folder

            os.chdir(Directory)

            if 'Windows-7' not in self.get_windows_version():

                name = QFileDialog.getOpenFileName(parent=self,directory = Directory,caption=self.tr("Select a file"),filter=self.tr('Image files (*.py)'),)
            else:
                name = QFileDialog.getOpenFileName(None, 'Open File', Directory, '*', '*', QFileDialog.DontUseNativeDialog)
            if name != '':
                file_name = name[0]
                self.ui.lineEdit_6.setText(file_name)
                self.old_script = file_name

    def new_script_button_clicked(self):
        #if self.active_file_name != None:
        #    Directory1 = os.path.basename(self.active_file_name)
        #    print("##################")
        #    print(Directory1)
        #else:
        #    Directory1 = self.All_Script_Folder
        #print('#####################')
        #print(Directory1)
        #os.chdir(Directory1)
        Directory1 = self.All_Script_Folder

        if 'Windows-7' not in self.get_windows_version():

            name = QFileDialog.getSaveFileName(parent=self, directory= Directory1,
                                               caption=self.tr("Select a file"), filter=self.tr('Image files (*.py)'), )
        else:
            name = QFileDialog.getSaveFileName(None, 'Open File',  Directory1, '*', '*',
                                               QFileDialog.DontUseNativeDialog)
        if name != '':
            file_name = name[0]
            self.ui.lineEdit_3.setText(file_name)
            self.new_script = file_name


    #def old_interface_clicked(self):







    #def new_interface_clicked(self):
    #    pass


    def save_clicked(self):
        pass

    def cancel_clicked(self):
        pass


    def get_windows_version(self):
        import platform
        ver = platform.platform()
        return ver
        #Windows - 7 - 6.1
        #.7601 - SP1





if __name__ == '__main__':
    app = QApplication(sys.argv)
    Script_Converter_Setup = Script_Converter_Setup_Class(app)
    #Script_Writer.showMaximized()
    sys.exit(app.exec_())