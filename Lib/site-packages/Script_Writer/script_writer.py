...
class Script_Writer_Class(QWidget):
    ...
    def Import_From_Computer(self):
        try:
            ...
            if 'Windows-7' not in self.get_windows_version():
                ...
                name = QFileDialog.getExistingDirectory(self, 'Select directory', QFileDialog.DontUseNativeDialog)
                print(name)
                #name = QFileDialog.getOpenFolderName(None, 'Open File', '.', '*', '*', QFileDialog.DontUseNativeDialog)
            if name != '':
                file_name = name
                if file_name != '':
                    src = file_name
    ...
    def Quick_run_was_clicked(self):
        try:
            ...
            #print('Starting process')
            self.process.start(r'C:\ImageScripter_2\python.exe', [r"C:\ImageScripter_2\Lib\site-packages\imagescripter\core\run.py"])
