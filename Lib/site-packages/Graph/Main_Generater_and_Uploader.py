...
from paths import MAX_CUTOFF,Converted_HTML_File,Converted_Temp_Path,Server_All_Videos_Folder_Location,Imageexelocation,Sandbox_location,Error_Text_Path,Desktop_location,ssh_host,ssh_user,ssh_password,ssh_port,Server_Entries_Folder_Location,isTesting,FAIL_PHP_FILE,PASS_PHP_FILE
from paramiko import SSHClient
import paramiko
import os
import time
import traceback
from Graph.Graph_Generator import Make_Graph,Upload_Graph_To_Server
...
class Uploader_Class():
  ...
  def put_file_in_folder_on_server(self,source_path,destination_path):
      ...
      for tries in range(100):
          try:
              ...
              self.scp.put(source_path,destination_path)
              was_Successful = True
              break
          except:
              traceback.print_exc()
              print('Failed to Put File In Folder On Server, Trying again... -> ', destination_path)
              Sleep(tries)

      if was_Successful == False:
          Mess = 'Did not put File In Folder On Server -> ' + destination_path
  ...
  def Get_List_Of_Files_On_Server(self,folder_on_server):
      try:
          List = []
          for filename in self.sftp.listdir(folder_on_server):
  ...
  def Get_Already_Uploaded_Entries(self):
      try:
          List_Of_Already_Uploaded_Entries = []
          try:
              self.sftp.listdir(Server_Entries_Folder_Location)
          except:
              Sleep(60)
              self.create_ssh_sftp_connection()
          ...
          for filename in self.sftp.listdir(Server_Entries_Folder_Location):
              List_Of_Already_Uploaded_Entries.append(filename)
          return List_Of_Already_Uploaded_Entries
      except Exception as e:
          print(e)
          traceback.print_exc()

  def Get_Already_Uploaded_Videos(self):
      try:
          List_Of_Already_Uploaded_Videos = []
          for filename in self.sftp.listdir(Server_All_Videos_Folder_Location):
...
=======
              #Sleep(i)
              if original_full_Path_avi == 'Too Short For Video':
                  print('Something went wrong with video, it was too short')
                  break
>>>>>>> c081bb69e12b18491d4289b239e59d54d296f368
              Sleep(i)
          else:
              print('Found video')
              Did_find_Video = True
  ...
  def Create_folders_for_all_entries_and_put_files_in(self):
      ...
      Get_Already_Uploaded_Entries_List = self.Get_Already_Uploaded_Entries()
      Get_Already_Uploaded_Videos = self.Get_Already_Uploaded_Videos()



      for Entry in Entries:
          Found_Video_On_Server = False

          ##############################################Get Information ##################
          index_time = str(Entry['index_time'])
          #if index_time == '1590605987.322966':
          ...
          ########################################
          if Passed == True:
            if the_pass_video_name_only_mp4 in Get_Already_Uploaded_Videos:
              Found_Video_On_Server = True
              print('Found Video ',the_pass_video_name_only_mp4)
          if Passed == False:
            if the_fail_video_name_only_mp4 in Get_Already_Uploaded_Videos:
              Found_Video_On_Server = True
              print('Found Video ',the_fail_video_name_only_mp4)

          #if the_fail_video_name_only_mp4 == 'FrankPC_Station__3_ITP8_fusion_lms__8.4.96.0__Fail__3493843889285504x.mp4':
          #    print('YESSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS')
          #else:
          #     print('NOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO')

          if index_time not in Get_Already_Uploaded_Entries_List or Found_Video_On_Server != True:
                      print('Did not find entry on server, Uploading -> ',index_time)
                      #######################################################
                      ##############################################CREATE DIR##########
                      ...
                      if Passed == False:
                          if isTesting == True:
                              ...
                              template_path = Entry['templet_img']
                      ...
                      if Found_Video_On_Server == True:
                          print('Video Exist On Server, Skipping  ')
                      else:
                          print('Video Doesnt Exist On Server, Uploading')
                          if Passed == False:
                              if isTesting == True:
              ...
              print('Found Entry and Video on server, Not Uploading -> ', index_time)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            print('finished making short graph')
            ...
            print('finished making long graph')
            print('Upload graph')
