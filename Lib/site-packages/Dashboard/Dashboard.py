...
class Dashboard_Class(QWidget):
    def __init__(self):
            ...
            image_changed_paths = [r"C:\Eluminate\System\ImageScripter\Lib\site-packages\Dashboard\Connected_Controllers\image.png"]
            self.image_changed_watcher = QtCore.QFileSystemWatcher(image_changed_paths)
            self.image_changed_watcher.fileChanged.connect(self.imageChanged)
            ...
            self.process2.readyReadStandardOutput.connect(self.stdoutReady)
            self.process2.readyReadStandardError.connect(self.stderrReady)
            self.process2.started.connect(lambda: self.started_process('Started!'))
            self.process2.finished.connect(lambda: self.finished_process('Finished!'))
            self.process2.start(Python_Exe_Location, [r"C:\Eluminate\System\ImageScripter\Lib\site-packages\Dashboard\Dashboard_Image_Server.py"])
    ...
    def imageChanged(self):
        try:
            ...
            Height = Height - 250
            Pixmap = QtGui.QPixmap(r"C:\Eluminate\System\ImageScripter\Lib\site-packages\Dashboard\Connected_Controllers\image.png")
            ...
            #self.ui.textBrowser.append('<h1>' + r"C:\Eluminate\System\ImageScripter\Lib\site-packages\Dashboard\Connected_Controllers\image.png" + '</h1><hr>')
            #self.ui.textBrowser.append('<img src="C:\Eluminate\System\ImageScripter\Lib\site-packages\Dashboard\Connected_Controllers\image.png" alt="Italian Trulli">')
    ...
    def stdoutReady(self):
        ...
        self.append(text)
        text = str(self.process2.readAllStandardOutput(), encoding="utf-8")
        self.append(text)
