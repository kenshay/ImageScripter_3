...
class Database_Viewer_Class(QWidget):
    ...
    def fail_and_play_video_button_clicked(self):
        ...
        subprocess.Popen(r"C:\ImageScripter_2\python.exe C:\ImageScripter_2\Lib\site-packages\imagescripter\core\Duel_Video_Player.py", shell=True)
    ...
    def get_and_fill_listview_with_data(self,Pass = "False",Controller_Build = '%',Station_Name = '%',Script_Name = '%'):
        ...
        Data = DataBase_Manager.get_data_from_databases(Target='Index_Time,Index_Time_String,Pass,Station_Name,Script_Name,Controller_Build',Pass = Pass,Controller_Build = Controller_Build,Station_Name = Station_Name,Script_Name = Script_Name)
    ...
    def list_view_selection_changed(self):
            ...
            try:
                ...
                if self.Pass == 'True':
                    ...
                    self.ui.label_14.setText('This Error Count: ' + str(Fail_Error_Amount))
                    self.ui.label_15.setText('Script Pass Count: ' + str(Pass_Amount))
                    self.ui.label_26.setText('Script Fail Count: ' + str(Fail_Amount))
                    self.ui.label_27.setText('Total Pass/Fail: ' + str(total))
                    self.Fail_Video = self.Fail_Video.replace(' ', '')
                    self.Pass_Video = entry[19]
                    self.Pass_Video = self.Pass_Video.replace(' ', '')
                    Client = entry[20]
