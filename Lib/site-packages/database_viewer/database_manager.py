...
class DataBase_Manager_Class():
    ...
    def get_entry_fron_Index_Time(self,Index_Time):
        Index_Time = str(Index_Time)
        Original_Index_Time = Index_Time
        print("Orgiginal",Original_Index_Time)
        entries = self.get_data_from_databases(Index_Time=Index_Time)
        try:
            entryTup = entries[0]
        except IndexError:
                try:
                    Index_Time = Original_Index_Time[:-1]
                    entries = self.get_data_from_databases(Index_Time=Index_Time)
                    entryTup = entries[0]
                except IndexError:
                    try:
                        Index_Time = Original_Index_Time[:-2]
                        entries = self.get_data_from_databases(Index_Time=Index_Time)
                        entryTup = entries[0]
                    except IndexError:
                        try:
                            Index_Time = Original_Index_Time[:-3]
                            entries = self.get_data_from_databases(Index_Time=Index_Time)
                            entryTup = entries[0]
                        except IndexError:
                            try:
                                Index_Time = Original_Index_Time[:-4]
                                entries = self.get_data_from_databases(Index_Time=Index_Time)
                                entryTup = entries[0]
                            except IndexError:
                                try:
                                    Index_Time = Original_Index_Time[:-5]
                                    entries = self.get_data_from_databases(Index_Time=Index_Time)
                                    entryTup = entries[0]
                                except IndexError:
                                        Index_Time = Original_Index_Time[:-6]
                                        entries = self.get_data_from_databases(Index_Time=Index_Time)
                                        entryTup = entries[0]
