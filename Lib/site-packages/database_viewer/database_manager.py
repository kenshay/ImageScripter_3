    ...
    def Remove_Ignore_From_Entry(self,Fail_Video):
        ...
        for DatabasePath in all_databases_list:
            if DatabasePath.endswith('.db'):
                ...
                conn = sqlite3.connect(sqlite_file)
                c = conn.cursor()
                command = "UPDATE Reports SET Ignore = 'False' WHERE Fail_Video like '" + Fail_Video + "'"
                print(command)
                c.execute(command)
                conn.commit()
                rows = c.fetchall()
...
if __name__ == '__main__':
    ...
    Fail_Video = """C:\Elan_Tools\Cloud_Drive\Videos_For_Database\FrankPC_Station__18_PC_fusion_lms__8.3.502.0__Fail__22662x.avi"""
    Data = DataBase_Manager.Set_Entry_To_Ignore(Fail_Video)
    #print(Data)
