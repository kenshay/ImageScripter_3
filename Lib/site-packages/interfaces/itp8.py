...
from interfaces.ITP_Functions import Enable_Root_SSH
...
class ITP8_Class(Device_Class):
    ...
    def Connect(self,portait = False):
        try:

            if Ping(self._ipadress_):
                with open(config_missing_var, 'r') as f:
                    String = f.read()
                with open(Settings_Location + r"Controller_Name.txt", 'r') as f:
                    controller_name = f.read()
                lastDIR = os.getcwd()
                os.chdir(IntefaceFiles)
                try:
                    if portait == False:
                        with open(controller_name, 'r') as f:
                            String = f.read()
                    elif portait == True:
                        with open(controller_name + '_p', 'r') as f:
                            String = f.read()
                    with open(configdate, 'w') as f:
                        f.write(String)
                    os.chdir(lastDIR)
                    remotepath = "/ELAN/config.dat"
                    ssh = paramiko.SSHClient()
                    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                    ssh.connect(self._ipadress_, username=self._username_, password=self._password_, port=self._port_)
                    sftp = ssh.open_sftp()
                    sftp.put(configdate, remotepath)
                    sftp.close()
                    ssh.close()
                    self.reboot()
                except Exception as e:
                    print('Controller Name Doesnt Exists. Please Add. Pass')
    ...
    def Unlock(self):
        server_ip = self._ipadress_
        port = self._port_
        root_username = self._username_
        root_password = self._password_
    ...
    def Software_Reset(self):
            ...
            for i in range(10):
                try:
                    ...
                    nbytes = 4096
                    command = """../opt/ELAN/elan_factory_reset.sh -f"""
                    stdin, stdout, stderr = self.client.exec_command(command)
                    Pass = True
                    break
                except paramiko.ssh_exception.SSHException as e:
                    ...
                    pass
            if Pass == False:
                raise ValueError('Having trouble Software Resetting')


    def reboot_old(self):
        Pass = False
        for atime in range(30):
            try:
                print('Rebooting ITP')
                self._start_connection_if_not_started_already_()
                print('Reboot ITP')
    ...
    def reboot(self):
        ...
        for turn in range(100):
            time.sleep(5)
            if Ping(self._ipadress_) == True:
                break
            else:
                print('100 / ',turn)
                if i >= 98:
                    raise ValueError('Interface never back came on')
                else:
                    time.sleep(1)
    ...
    def NEW_get_image_of_screen(self):
        for atime in range(100):
            try:
                ...
                # Flip
                picture = Image.open(screenshot_file_itp8)
                picture.rotate(90, expand=True).save(screenshot_file_itp8)
                picture.close()
                break
                ########################################################################################
            except Exception as e:
    ...
    def _click_(self,y,x):
        #old_click_new_connection_each_time
        print("_click_")
        x = str(x)
        y = str(y)
        nbytes = 4096
