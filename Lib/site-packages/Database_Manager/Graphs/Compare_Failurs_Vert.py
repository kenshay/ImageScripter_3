...
for Station_Name in Stations_List:
    if Station_Name not in IGNORE_LIST:
        ...
        for rect, pass_Fail_Binary,duration in zip(PassBar,PassesFailsListOfListsFromFuction,List_of_script_Durations):
            ...
            width = x + width
            #width = width / 100 * 95
            ax.text(width,y + (height / 2 ),str(pass_Fail_Binary[-6:]))
        ...
        plt.legend(loc='best')
        plt.title('Hours (' + str(convert(Total_Time_In_Seconds)) + ')             ' + Station_Name + '             ' + Dictionary_Database_Manager.Active_Build_without_EXE,pad=30)
