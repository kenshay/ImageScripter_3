...
for Station_Name in Stations_List:
    if Station_Name not in IGNORE_LIST:
        ...
        plt.bar(ind, Fails, width=1, label='Fail', color='#ad4949')
        plt.bar(ind, Passes, width=1, label='Pass', color='#37c472', bottom=Fails)
        plt.xticks(ind, scripts)
        plt.ylabel("Amount")
        plt.xlabel("Scripts")
        plt.xticks(rotation=90)
        plt.legend(loc="upper right")
        plt.title(Station_Name + '             ' + Dictionary_Database_Manager.Active_Build_without_EXE)
