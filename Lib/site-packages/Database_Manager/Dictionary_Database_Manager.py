...
def is_there_any_Scripts_entries_for_build_and_station(Branch,Station):
    try:
        SCRIPT_NAME_LIST = []
        print('Getscripts_per_build')
        ...
        for database_file_name in List_of_databases:
            ...
            if database_file_name.endswith('.json'):
                if Branch in database_file_name:
                    if Station in database_file_name:
                        ###################################
                        ##################################
                        with open(database_file_name, 'rb') as f:
                                LIST = pickle.load(f)
                        for dicta in LIST:
                            script_name = dicta['script_name'].split(':')[1]
        ...
        if SCRIPT_NAME_LIST != []:
            ...
            print('None Available for ',Branch,'  -->  ',Station)
            return False
...
class Dictionary_Database_Manager_Class():
    ...
    def get_all_station_names(self):
            ...
            for database_file_name in List_of_databases:
                # print(database_file_name)
                if database_file_name.endswith('.json'):
                    Station_Name = database_file_name.split('__')[0]
    ...
    def get_last_run_script_by_station(self,station):
        try:
            List_of_scripts = []
            for entry in self.Cached_List_Of_Entries:
                    station_name = entry['station_name']
                    station_name = station_name.split(':')[1]
                    if station_name == station:
                        List_of_scripts.append(entry['index_time'])
            Last = sorted(List_of_scripts)[-1]
            return Last
        except:
            return 0
    ...
    def get_list_of_logs_by_station(self,Station):
        Log_List = []
        for i in self.Cached_List_Of_Entries:
                ...
                if station_name == Station:
                    ...
                    #############
                    Memory_Start = i['memory_used_start']
                    Memory_Start = Memory_Start.split(':')[1]

                    Memory_End = i['memory_used_end']
                    Memory_End = Memory_End.split(':')[1]
                    ...
                    InnerList.append(build)
                    InnerList.append(Pass)


                    InnerList.append(Memory_Start)
                    InnerList.append(Memory_End)
...
if __name__ == "__main__":
    ...
    #Dictionary_Database_Manager.get_scripts_per_build('8.6.1.40','Quick_Station')
    is_there_any_Scripts_entries_for_build_and_station('8.6.1.40','Frank_sc300')
    #for i in List:
    #    print(i)
