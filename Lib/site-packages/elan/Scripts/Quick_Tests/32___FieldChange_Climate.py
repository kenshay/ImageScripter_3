
from elan import *
#Finished 3/6/19
#DONE
Configurator.Reset(start=True)
Configurator.Start(password = '')
########################################Climate###########################
Configurator.climate.Click()
################com device
Configurator.communicationdevices.RightClickType('a')
AddNewDevice.ListView.Select(0, 'Virtual HVAC Network')
AddNewDevice.ListBox.Select(0, 'Serial Port')
AddNewDevice.ComboBox.Select(0,1)
AddNewDevice.PushButton.Click('OK')
####################
Configurator.discoverdevices.Click()
#############################
######################################CrashCheck
Configurator.system.Click()
Configurator.climate.Click()
Configurator.virtualhvacnetwork.Click()
Configurator.Edit.SetAllText('Test')AddNewDevice.ListView.Select(0, 'Virtual HVAC Network')
Configurator.ComboBox.SelectAllWithIndex('1')
Configurator.apply.Click()
Configurator.media.Click()
Configurator.CloseAndClean()
