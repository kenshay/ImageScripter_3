

from elan import *


Start_Image_Show()


Viewer.Start(quick = True)
GetBackTOHomeScreenPC()
Viewer.media10items.Click()
Sleep(2)
try:
    Viewer.audiogroup.Wait()
    Viewer.audiogroup.Click()
    Sleep(2)
except:
    try:
        Viewer.media10items.Click()
        Sleep(3)
        Viewer.audiogroup.Click()
        Sleep(2)
    except:
        Sleep(3)
        Viewer.media10items.Click()
        Sleep(3)
        if not Viewer.audiogroup.Exists():
            Viewer.s86.Click()
            Sleep(2)
            Viewer.shudder.Click()
            Sleep(3)
            Viewer.audiogroup.Wait()
        Viewer.audiogroup.Click()
        Sleep(2)

#DONE
#S86
#######################################################
Viewer.s86.Click()
#####################################################
sleep(3)
Viewer.zoneoff2.Click()
try:
    Viewer.smalloffbottomleft.Click()
except:
    Viewer.zoneoff2.Click()
    Viewer.smalloffbottomleft.Click()
sleep(1)
###########################################################
Viewer.lmsicon.Click()
Sleep(2)
Viewer.pandoraicon.Click()
Sleep(2)
Viewer.yourstations.Click()
Sleep(2)
Viewer.dickdaleradio.Click()
Sleep(2)
Viewer.bluearrow.Click(threshold = .92)
Sleep(2)
Viewer.lmsback.Click()
Sleep(2)
Viewer.lmsback.Click()
Sleep(2)
try:
    Viewer.pandoradickdaleradio.Wait(seconds=10)
except:
    try:
        Viewer.lmsback.Click()
        Sleep(2)
        Viewer.lmsback.Click()
        Sleep(2)
        Viewer.pandoradickdaleradio.Wait()
    except:
        Viewer.pandoradickdaleradio.Wait()
Viewer.mylibraryicon.Click()
Sleep(2)
Viewer.albums.Click()
Sleep(2)
Viewer.hamburg.Click()
Sleep(2)
Viewer.bluearrow.Click(threshold = .92)
Sleep(2)
Viewer.lmsback.Click()
Sleep(2)
Viewer.lmsback.Click()
Sleep(2)
try:
    Viewer.pearljamcover.Wait(seconds = 10)
except:
    try:
        Sleep(2)
        Viewer.lmsback.Click()
        Sleep(2)
        Viewer.pearljamcover.Wait(seconds=10)
    except:
        Viewer.pearljamcover.Wait(seconds=10)
Viewer.lmssettings.Click()
Sleep(15)
Viewer.tidal.Click(xoffset=470,yoffset=0)
Sleep(2)
Viewer.lmsback.Click()
Sleep(2)
Viewer.tidalicon.Wait()
Viewer.lmssettings.Click()
Sleep(2)
Viewer.tidal.Click(xoffset=470,yoffset=0)
Sleep(2)
Viewer.lmsback.Click()
Sleep(2)
try:
    Viewer.tidalicon.WaitVanish(seconds = 10)
except:
    Sleep(2)
    Viewer.lmssettings.Click()
    Sleep(2)
    Viewer.tidal.Click(xoffset=470, yoffset=0)
    Sleep(2)
    Viewer.lmsback.Click()
    Sleep(2)
    Viewer.tidalicon.WaitVanish(seconds=10)
    Sleep(2)
Viewer.lmsoffbig.Click()
sleep(1)
Viewer.lmsoffoff.Click()


Viewer.shudder.Click(threshold = .92)
Sleep(2)
Viewer.homeicon9.Click(threshold = .92)
Sleep(2)
Viewer.homeicon10items.Wait(seconds = 30)
Viewer.homeicon10items.Click(threshold = .92)
Sleep(2)
#Viewer.shudder.Click(threshold = .92)
#Viewer.homeicon9.Click()
#Viewer.homeicon9.Click()
Viewer.CloseAndClean()
Close_Image_Show()