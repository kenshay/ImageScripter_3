...
#Memory_In_Free = SSH_Manager.get_percent_of_memory_free()
Memory_In_Use = float(SSH_Manager.get_percent_of_memory_In_Use())

Pass = False
for i in range(100):
    try:
        print('Memory_In_Use',Memory_In_Use)
        Pass = True
        break
    except Exception as e:
        print(e)
if Pass == False:
    raise ValueError("Could not get memory")
if Memory_In_Use >= 40:
    raise  ValueError('Memory is higher than 40% -> ',str(Memory_In_Use) + '%')
else:
    print('Memory is under 40% -> ',str(Memory_In_Use) + '%')
