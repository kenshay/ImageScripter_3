...
try:
    Viewer.redconnecting.Wait(seconds = 20,threshold = .92)
...
if Viewer.pooltopleft.GetDominantColorClosest(threshold=.83) == 'mediumblue':
    Viewer.pooltopleft.Click(threshold=.83)
sleep(10)
if Viewer.spatopright.GetDominantColorClosest(threshold=.83) == 'mediumblue':
...
Viewer.spatempnothing.Wait(threshold = .92)

Viewer.pooltopleft.Click(threshold=.83)
try:
    Viewer.tempset2.Wait(seconds = 20,threshold=.83)
except:
