...
def GoHome():
    ...
    command = r'http://_IP_/clientstoroot'
    command = command.replace('_IP_',IP)


    print('All Screens Go Home Start ',IP)
    import urllib.request
    with urllib.request.urlopen(command) as response:
        html = response.read()
    print('All Screens Go Home Finish')

def StayOn():
    IP = Variable_File_Getter.getVariableFromFile('Controller_IP')
    command = r'http://_IP_/disablescreenoff'
    command = command.replace('_IP_', IP)

    print('All Screens Stay On Start ',IP)
    import urllib.request
    with urllib.request.urlopen(command) as response:
        html = response.read()
    print(html)
    print('All Screens Stay On Finish')
    ...
    for i in range(2):
        try:
            ...
            try:
                ...
                Itp12.elanbiglogo.Wait(seconds=1)
                Pass = True
                break
                                    ...
                                    try:
                                        ...
                                        try:
                                            ...
                                            try:
                                                ...
                                                try:
                                                    Itp8p.blackbottomright.Wait(seconds=2)
                                                    Itp8p.blackbottomright.Click(xoffset=-1498, yoffset=504)
                                                    Itp8p.shudder.Click()
                                                    Itp8p.videohighlighted3.Click()
                                                    Itp8p.settingswheel.Wait()
                                                    Pass = True
                                                    break
                                                except Exception as e:
